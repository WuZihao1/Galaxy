...
void main()
{
	...
	if(normal_flag==1)
	{
		vec3 normal = texture(myTextureSampler_2,UV.xy).rgb;
		...
		float brightness_2 = dot(lightVectorWorld_2 , normal);
		...
		vec3 MaterialAmbientColor = texture2D(myTextureSampler_1,UV).rgb*0.5 + texture2D(myTextureSampler_2,UV).rgb*0.5;
		vec3 MaterialDiffuseColor = texture2D(myTextureSampler_1,UV).rgb*0.5 + texture2D(myTextureSampler_2,UV).rgb*0.5;
		vec3 MaterialSpecularColor = lightColor ;
		vec3 color_1 = MaterialAmbientColor* ambientLight   + MaterialDiffuseColor * clamp(diffuseLight,0,1) * coefficient_d + MaterialSpecularColor* s * coefficient_s  ;
		
		s_2 = pow(s_2,coefficient_s_2[0]);
		vec3 MaterialAmbientColor_2 = texture2D(myTextureSampler_1,UV).rgb*0.5 + texture2D(myTextureSampler_2,UV).rgb*0.5;
		vec3 MaterialDiffuseColor_2 = texture2D(myTextureSampler_1,UV).rgb*0.5 + texture2D(myTextureSampler_2,UV).rgb*0.5;
		vec3 MaterialSpecularColor_2 = lightColor ;
		vec3 color_2 = MaterialAmbientColor* ambientLight   + MaterialDiffuseColor_2 * clamp(diffuseLight_2,0,1) * coefficient_d_2 + MaterialSpecularColor_2* s_2 * coefficient_s_2  ;
	...
	else
	{
		...
		vec3 color = color_2*0.5+color_1*0.5;
